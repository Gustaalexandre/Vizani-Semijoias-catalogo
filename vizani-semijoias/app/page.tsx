...
export default function Home() {
  ...
  const lista: Produto[] = produtos.filter((p: Produto) => {
  ...
  return (
    <>
      ...
      {/* Hero */}
      <div style={{
        background: 'linear-gradient(135deg,#fdf6e3 0%,#f5e9c4 60%,#faf9f7 100%)',
        padding: '48px 24px 36px',
        textAlign: 'center',
      }}>
        <p style={{ fontSize: 11, letterSpacing: 4, color: '#C4952A', textTransform: 'uppercase', marginBottom: 10 }}>
          Coleção Exclusiva
        </p>
        <h1 style={{ color: '#8B6914', letterSpacing: 4, fontWeight: 300, fontSize: 32, marginBottom: 10 }}>
        ...
        <p style={{ color: '#A07828', fontStyle: 'italic', fontSize: 14 }}>
          Semijoias banhadas a ouro • Qualidade premium
        </p>
      </div>
            ...
            {[
              ...
              <button
                ...
                style={{
                  padding: '8px 16px',
                  borderRadius: 8,
                  border: filtro === key ? '1px solid #8B6914' : '1px solid #e8d5a3',
                  background: filtro === key ? '#8B6914' : '#fff',
                  color: filtro === key ? '#fff' : '#8B6914',
                  fontSize: 13,
                  cursor: 'pointer',
            ...
            <div key={p.id} className="col-6 col-md-4 col-lg-3">
              <div style={{
                ...
                onMouseEnter={e => {
                  ...
                  (e.currentTarget as HTMLDivElement).style.boxShadow = '0 8px 32px rgba(139,105,20,0.13)';
                }}
                onMouseLeave={e => {
      ...
      <div style={{ background: '#fdf6e3', borderTop: '1px solid #e8d5a3', padding: '32px 24px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 20, marginBottom: 16 }}>
          <a href={`https://instagram.com/${INSTAGRAM}`} target="_blank" rel="noopener noreferrer"
            style={{ display: 'flex', alignItems: 'center', gap: 6, textDecoration: 'none', color: '#8B6914', fontSize: 13 }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="#C4952A">
              <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
            </svg>
            @{INSTAGRAM}
          </a>
          <a onClick={() => window.open(`https://wa.me/${WA_NUMBER}`, '_blank')}
            style={{ display: 'flex', alignItems: 'center', gap: 6, textDecoration: 'none', color: '#8B6914', fontSize: 13, cursor: 'pointer' }}>
        ...
        >
          ...
          >
            ...
            <div style={{ position: 'relative', width: '100%', height: 280, background: '#fdf6e3', flexShrink: 0 }}>
              <Image src={modal.foto} alt={modal.nome} fill style={{ objectFit: 'contain' }} />
              <button
                onClick={() => setModal(null)}
                style={{
            ...
            <div style={{ padding: '20px 20px 28px', overflowY: 'auto' }}>
              <span style={{ fontSize: 10, letterSpacing: 2, color: '#C4952A', textTransform: 'uppercase', fontWeight: 600 }}>
                {modal.categoria}
              </span>
              <h5 style={{ color: '#3d3020', marginTop: 8, marginBottom: 10, fontSize: 20 }}>
                {modal.nome}
              </h5>
              <div style={{ width: 40, height: 1, background: 'linear-gradient(90deg,#C4952A,transparent)', marginBottom: 12 }} />
              <p style={{ color: '#5a4a30', fontSize: 14, lineHeight: 1.7, marginBottom: 16 }}>
                {modal.descricao}
              </p>
              <p style={{ color: '#8B6914', fontSize: 26, fontWeight: 700, marginBottom: 20 }}>
                {modal.preco}</p>
              <button
